...
class DDQNAgent(object):
    ...
    def choose_action(self, state):
        ...
        if self.memory.mem_cntr < 500:  # Warmup phase
...
class Brain:
    ...
    def createModel(self):
        ...
        input_layer = tf.keras.Input(shape=(self.NbrStates,))
        dense1 = tf.keras.layers.Dense(256, activation='relu')(input_layer)

        value = tf.keras.layers.Dense(1)(dense1)
        advantage = tf.keras.layers.Dense(self.NbrActions)(dense1)
        q_vals = tf.keras.layers.Lambda(lambda a: a[0] + (a[1] - tf.reduce_mean(a[1], axis=1, keepdims=True)))([value, advantage])

        model = tf.keras.Model(inputs=input_layer, outputs=q_vals)
